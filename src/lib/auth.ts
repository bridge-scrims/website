...
export function getPositionLevelValue(pos?: string) {
    return Object.keys(POSITION_ROLES)
        .reverse()
        .indexOf(pos ?? "")
}
...
type VerifiedOrRedirect =
    | { redirect: Redirect; verified?: undefined }
    | { redirect?: undefined; verified: GuildMember }
export async function verifySessionOrRedirect(
    context: GetServerSidePropsContext,
    positionLevel?: PositionLevel
): Promise<VerifiedOrRedirect> {
...
export async function verifySessionOr401Response(
    req: NextApiRequest,
    res: NextApiResponse,
    positionLevel?: PositionLevel
) {
